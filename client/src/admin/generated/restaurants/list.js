...
const restaurantsColumns = [
    ...
    },
    {
        ...
        accessor: "categoryID",
        Cell: data => (
            <IMForeignKeyTableCell id={data.value} apiRouteName="category" titleKey="title" />
        )
    },
    {
        Header: "Description",
...
function ActionsItemView(props) {
    ...
    const handleView = (item) => {
        const viewPath = "/admin/restaurant/" + item.id + '/view'
        history.push(viewPath)
    }

    const handleEdit = (item) => {
        const editPath = "/admin/restaurant/" + item.id + '/update'
        history.push(editPath)
    }

    const handleDelete = async (item) => {
        if (window.confirm('Are you sure you want to delete this item?')) {
            const response = await fetch(baseAPIURL + 'restaurant/' + item.id, {
...
function RestaurantsListView(props) {
    ...
    useEffect(() => {
        ...
        setRestaurants(data.slice(startRow, endRow))
                    ...
                    <Col md="12">
                        <div className="Card">
                            <div className="CardHeader">
                                <a className="Link AddLink" href="/admin/restaurant/add">Add New</a>
                                <h4>Restaurants</h4>
                            </div>
                                ...
                                <div className="TableContainer">
                                    <table className="Table" {...getTableProps()}>
                                        ...
                                        <tbody {...getTableBodyProps()}>
                                            ...
                                            <tr>
                                                {isLoading ? (
                                                    <td colSpan={restaurantsColumns.length - 1}>
                                                        <p>
                                                            Loading...
                                                        </p>
                                                    </td>
