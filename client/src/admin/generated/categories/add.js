...
const AddNewCategoriesView = () => {
    ...
    const createCategories = async (data, setSubmitting) => {
        console.log(JSON.stringify(data));
        setIsLoading(true);
        const response = await fetch(baseAPIURL + 'category/add', {
        ...
        if (!values.order) {
            errors.order = 'Field Required!'
                ...
                onSubmit={(values, { setSubmitting }) => {
                    createCategories(values, setSubmitting);
                }}
            >
                {({
                values,
...
export default AddNewCategoriesView;