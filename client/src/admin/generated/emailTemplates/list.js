...
import { IMLocationTableCell, IMImagesTableCell, IMDateTableCell, IMForeignKeyTableCell, IMAddressTableCell } from '../../ui/IMTable';
...
function ActionsItemView(props) {
    ...
    const handleView = (item) => {
        const viewPath = "/admin/template/" + item.id + '/view'
        history.push(viewPath)
    }

    const handleEdit = (item) => {
        const editPath = "/admin/template/" + item.id + '/update'
        history.push(editPath)
    }

    const handleInsert = (item, callback) => {
    ...
    const handleDelete = async (item) => {
        if (window.confirm('Are you sure you want to delete this item?')) {
            const response = await fetch(baseAPIURL + 'template/' + item.id, {
    ...
    return (
        <div className="inline-actions-container">
            ...
            {insertCallback &&
                <button onClick={() => handleInsert(data.row.original, insertCallback)} type="button" id="tooltip476609793" className="btn-icon btn btn-info btn-sm"><i className="fa fa-file-code-o"></i></button>
            }
        </div>
    )
}

function TemplatesListView(props) {
    ...
    const {
    ...
    } = useTable(
        {
            columns,
            data: templates,
            insertCallback: props.insertCallback,
    ...
    useEffect(() => {
        ...
        setTemplates(data.slice(startRow, endRow))
                    ...
                    <Col md="12">
                        <div className="Card">
                            <div className="CardHeader">
                                <a className="Link AddLink" href="/admin/template/add">Add New</a>
                                <h4>Templates</h4>
                            </div>
                                ...
                                <div className="TableContainer">
                                    <table className="Table" {...getTableProps()}>
                                        ...
                                        <tbody {...getTableBodyProps()}>
                                            ...
                                            <tr>
                                                {isLoading ? (
                                                    <td colSpan={templatesColumns.length - 1}>
                                                        <p>
                                                            Loading...
                                                        </p>
                                                    </td>
