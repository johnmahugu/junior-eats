...
import { IMColorPickerSketch } from '../../../common'
...
class IMRichTextEditor extends React.Component {
    ...
    _colorPickerChange = color => {
        ...
        this.onChange(RichUtils.toggleInlineStyle(this.state.editorState, 'COLORPICKER'))
    }
    ...
    _myBlockStyleFn = contentBlock => {
        ...
        switch (type) {
            case 'blockquote': return "myBlockquote"
            default: return null
        }
    }
    ...
    _myKeybindingFn = event => {
        if (event.keyCode === 88 && event.shiftKey && KeyBindingUtil.hasCommandModifier(event)) {
            return "strikethrough"
        }
        if (KeyBindingUtil.hasCommandModifier(event) && event.shiftKey && event.key === 'h') {
            return 'highlight';
        }
        if (event.keyCode === 9) { //managing tab(adds a tab only to lists)
    ...
    _handleKeyCommand = command => {
        var myEditorState = RichUtils.handleKeyCommand(this.state.editorState, command);

        if (!myEditorState && command === 'strikethrough') {
            myEditorState = RichUtils.toggleInlineStyle(this.state.editorState, 'STRIKETHROUGH');
        }
        if (!myEditorState && command === 'highlight') {
            myEditorState = RichUtils.toggleInlineStyle(this.state.editorState, 'HIGHLIGHT');
        }
    ...
    render() {
        ...
        const myStyleMap = {
            CODE: {  //Here we can add/overwrite styles (bold, italic, etc)
        ...
        const exporterOptions = {
            ...
            blockRenderers: {
                atomic: (block) => {
                    const entity = editorState.getCurrentContent().getEntity(
                        block.getEntityAt(0)
                    );
            ...
            blockStyleFn: (block) => {
                ...
                switch (type) {
                    case 'blockquote': return {
                        style: { ///use the myBlockquote style
        ...
        this.innerHTML = stateToHTML(editorState.getCurrentContent(), exporterOptions);
