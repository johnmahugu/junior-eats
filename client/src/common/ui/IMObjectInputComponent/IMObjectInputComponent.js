...
function IMObjectInputComponent(props) {
    ...
    const { handleClick, handleDelete, initialData, data, nonModifiable, valuePlaceholder, keyPlaceholder, } = props
    ...
    const dataItems = myData && Object.keys(myData).length ? (Object.keys(myData).map(key => {
        if (typeof myData[key] === "string" || typeof myData[key] === "number") {
            return (
                <li className="FieldStyling">{key}:
                    <div className="ObjectInputs">
                        <input
                            type="text"
                            className="ObjectInputStyling"
                            value={myData[key]}
                            onChange={(event) => { handleClick(key, event.target.value); changeObjectElement(key, event.target.value) }} />
                        <button 
        ...
        <div style={{paddingBottom: ((dataItems) ? inputPadding : '40px')}}>
            ...
            </span>
            {
                ...
                (<div className="InputContainer">
                    ...
                    />
                    <button onClick={onClick} type="button" className="btn-icon btn btn-secondary btn-sm ButtonStyling">
