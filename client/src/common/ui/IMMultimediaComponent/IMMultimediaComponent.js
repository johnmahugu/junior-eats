...
function IMMultimediaComponent(props) {
    ...
    return (
        <div className={allContainerClasses} onClick={onClick}>
            ...
            {dismissable && (
                <IMDismissButton className="MediaDismissContainer" onPress={onDismiss} />
            )}
            {openable && mediaType === 'image' && (
                <IMModal className="MediaModal" visible={isOpen} onDismiss={onModalDismiss}>
                    <div className="MediaModalImgContainer">
                        <img className="MediaFullscreen"
                            src={src}
                        />
                    </div>
                </IMModal>
