...
function IMForeignKeysIdComponent(props) {
    ...
    var urls = ids && ids.length && ids.map(id => (baseAPIURL + apiRouteName + '/' + id))
    const viewPath = ids && ids.length && ids.map(id => ("/admin/" + apiRouteName + "/" + id + '/view'))

    return (
        <div className="ForeignKeysComponent">
            {viewPath && viewPath.length > 0 &&
                viewPath.map( (el, index) => {
                    return (<li><a href={el}>{data[index][titleKey]}</a></li>)
                })
            }
        </div>
    )
