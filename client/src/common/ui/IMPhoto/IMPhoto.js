...
function IMPhoto(props) {
    ...
    return (
        <div className={allContainerClasses} onClick={onClick}>
            <img className={allClasses}
                src={src}
            />
            {dismissable && (
                <IMDismissButton className="PhotoDismissContainer" onPress={onDismiss} />
            )}
            {openable && (
                <IMModal className="PhotoModal" visible={isOpen} onDismiss={onModalDismiss}>
                    <div className="PhotoModalImgContainer">
                        <img className="PhotoFullscreen"
                            src={src}
                        />
                    </div>
                </IMModal>
